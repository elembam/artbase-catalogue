# ArtBase Wikidata Application Profile

*The formal specification for how ArtBase uses Wikidata — what we require, what we recommend, how we structure statements, and how compliance is validated.*
...
1. **Defines what ArtBase requires** from a Wikidata entry before linking to it from an ArtBase passport
2. **Defines what ArtBase contributes** when working on entries during cataloguing
3. **Defines how compliance is validated** so the pipeline can flag issues automatically

This document is the spec. It's the contract between ArtBase's catalogue and Wikidata. It also serves as the model that partners adopt and that the property proposals reference.
...
- **Artists** — humans (or, occasionally, anonymous groups) responsible for creating artworks in ArtBase records
...
An entry can be linked from an ArtBase passport only if it has all of the following. If any are missing, the pipeline flags it; the cataloguer adds the missing statements (with references) before linking.
...
Added when sources allow. Their presence raises an entry's ArtBase compliance score but does not block linking.
...
| `P800` notable work | Individual artwork items (linked to ArtBase passports if available) |
...
ArtBase considers an artist entry "fully cross-referenced" only when at least three of these external IDs are present (or have been verifiably searched for and confirmed absent):
...
When `P-ArtBase-Artist-ID` is approved as a Wikidata property (see Part 4), it becomes a *required* identifier for any artist with an ArtBase record.

### Reference patterns

Every statement contributed by ArtBase has at least one reference following one of these standard patterns:
...
**Pattern C — Citation of ArtBase as source.** Used only when ArtBase's house records are the primary source (typical for under-documented contemporary artists).

```
Statement: [any]
References:
  ▸ P248 (stated in): Q[ArtBase item, once approved]
  ▸ P854 (reference URL): https://artbase.eu/a/AR9D2HK4Q8
  ▸ P813 (retrieved): 2026-05-27
```

### Rank usage

ArtBase uses Wikidata's statement-rank system explicitly:
...
When relevant, ArtBase adds qualifiers:
...
### Cross-reference to ArtBase

When `P-ArtBase-Passport-ID` is approved as a Wikidata property, every artwork in ArtBase's catalogue with a corresponding Wikidata item gets this identifier added. This is the formal link that lets a Wikidata user click through to the ArtBase passport.

---

## Part 3 — EntitySchema

ArtBase publishes its profile as a formal Wikidata EntitySchema using Shape Expressions (ShEx). The schema lives on Wikidata at `EntitySchema:E[number]` once submitted.

The schema is machine-readable: anyone can validate any artist or artwork entry against it using the Wikidata schema validator. Tools and dashboards can produce reports like "X% of items linked from ArtBase are fully compliant with the ArtBase artist profile."
...
ArtBase proposes two new properties for Wikidata. Each follows the standard property proposal process (community discussion, vote, technical creation).

### Property 1 — ArtBase Artist ID

- **Label**: ArtBase artist ID
- **Description**: Identifier for an artist in the ArtBase registry of art in private and corporate collections
- **Datatype**: External identifier
- **Format**: `^AR[A-Z0-9]{8}$` (matching ArtBase's artist ID scheme)
- **Formatter URL**: `https://artbase.eu/a/$1`
- **Allowed values**: One per artist
- **Source**: ArtBase Cataloguing Services

Justification: Each authority cross-reference adds a piece of infrastructure to the Wikidata graph. ArtBase's records will, over time, include thousands of artist entries — especially in the corporate and private-collection space where Wikidata coverage is currently weak. A dedicated property allows future-proof cross-referencing and prevents the use of generic `P973` (described at URL) which is a less useful structural pattern.

### Property 2 — ArtBase Passport ID

- **Label**: ArtBase passport ID
- **Description**: Identifier for an artwork in the ArtBase registry
- **Datatype**: External identifier
- **Format**: `^AB[A-Z0-9]{8}$`
- **Formatter URL**: `https://artbase.eu/p/$1`
- **Allowed values**: One per artwork (artworks with multiple historical IDs use the canonical one)
- **Source**: ArtBase Cataloguing Services

Justification: The artwork-level equivalent of the artist ID, allowing Wikidata entries for artworks to cross-reference their ArtBase passports.

### Submission timing

Property proposals require justification through demonstrated use. The right time to submit:

- **Artist ID**: After ArtBase has ~50 artist records and at least 20 of those have made improvements to existing Wikidata entries — so the community sees concrete contribution before considering a new property. Realistic timing: end of Year 1.
...
| **Pre-publication** | Missing required statements; not yet linkable from an ArtBase passport |

The ArtBase contribution log records compliance level per entry at the time of last touch, allowing year-over-year reporting on quality improvement.

---

## Part 6 — Alignment with existing WikiProjects

ArtBase's profile aligns with two existing Wikidata WikiProjects:

### Visual arts WikiProject
- Defines artist entity conventions (occupations, movements, education patterns)
- ArtBase profile is a superset — adds required cross-reference IDs and statement-sourcing requirements
- We follow their conventions for properties not specifically required by ArtBase
...
- ArtBase profile aligns and adds ArtBase-specific identifiers

Alignment means: where existing WikiProjects have made decisions, we follow them. Where they're silent or under-specified, ArtBase defines our usage. Where we want changes, we propose them through the WikiProject's discussion process, not unilaterally.
...
6. **Bulk re-validation** — existing ArtBase contributions re-checked against new profile
...
- **Wikidata users see ArtBase as a serious contributor.** A documented profile published as an EntitySchema is the calling card that distinguishes serious institutional contributions from drive-by edits.
...
- **The relationship is reciprocal, not dependent.** ArtBase doesn't depend on Wikidata to function; Wikidata doesn't depend on ArtBase. We exchange data under documented terms.
...
*This profile is the schema layer of ArtBase's Wikidata relationship. It is independent of the ArtBase passport format itself (which uses Object ID / LIDO / EODEM) and serves as the bridge between our internal records and the public knowledge graph.*
